...
// runWithLookupCaptureStdout swaps os.Stdout for the duration of runWithLookup.
func runWithLookupCaptureStdout(t *testing.T, args []string) (code int, stdout string) {
	t.Helper()
	r, w, err := os.Pipe()
	if err != nil {
		t.Fatal(err)
	...
	old := os.Stdout
	os.Stdout = w
	code = runWithLookup(args, nil, nil)
...
func TestRunWithLookup_Version(t *testing.T) {
	code, out := runWithLookupCaptureStdout(t, []string{"rmig", "version"})
	if code != 0 {
...
func TestRunWithLookup_VersionJSON(t *testing.T) {
	code, out := runWithLookupCaptureStdout(t, []string{"rmig", "--json", "version"})
	if code != 0 {
		t.Fatalf("exit code = %d, want 0", code)
	}
	if !strings.Contains(out, `"version"`) || !strings.Contains(out, `"commit"`) {
		t.Fatalf("stdout = %q, want JSON with version and commit", out)
	}
}

func TestRunWithLookup_VersionSkipsEnvFile(t *testing.T) {
	code, out := runWithLookupCaptureStdout(t, []string{"rmig", "--env", "/nonexistent/.env", "version"})
	if code != 0 {
		t.Fatalf("exit code = %d, want 0 (version must not read .env)", code)
	}
	s := strings.TrimSpace(out)
	if !strings.HasPrefix(s, "rmig ") {
		t.Fatalf("stdout = %q, want successful version line despite bogus --env", s)
	}
}
