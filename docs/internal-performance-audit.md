...
- **Object map keys:** use [`types.NormalizedKey`](internal/types/key.go) from `internal/types/key.go` for `schema/kind/name` map keys; do not hand-roll `strings.ToLower(schema+"/"+...)`. `NormalizedKey` joins with a [`strings.Builder`](internal/types/key.go) then applies a single `strings.ToLower` on the full path (Unicode parity with legacy chained `+`). For triggers, [`fs.Object.ParentNormalizedKey`](internal/fs/layout.go) is populated in [`Scanner.newObject`](internal/fs/scanner.go) when `kind == "triggers"` and `ParentName` is set, so [`internal/diff/diff.go`](internal/diff/diff.go) can reuse the parent table key without recomputing it (fallback still recomputes if the field is empty, e.g. hand-built test fixtures). [`types.BuildINQuery`](internal/types/chunk.go) expands `@p` lists by scanning the template once (no intermediate string holding only the placeholder list when `len(keys) > 0`).
...
2. `internal/db/inspector_impl.go` derives schema and object scope from `fs.Layout`, deduplicates names with maps, probes `sys.databases.compatibility_level` once per inspector instance, and prefers static `OPENJSON` SQL for schema, object, and column reads while keeping chunked `IN (...)` queries as the fallback.
...
| Lazy file and Git metadata caching | `CachedFile` in `internal/fs/layout.go` uses `sync.Once` for content, checksum, and Git metadata, `Content()` aliases the `os.ReadFile` buffer instead of making a second full string copy, and `Checksum()` now hashes bytes directly without forcing `Content()` to populate its cache first; `internal/fs/scanner.go` keeps repo-state keyed Git preload cache plus shared checksum cache so repeated scans of the same repository tree can reuse batched `git log` and checksum results; eager checksum preload is limited to objects because transitions/checks are not part of the hot diff comparison path | Confirmed by code |
| Cached layout rebuild between repeated scans | `internal/fs/scanner.go` now stores per-root layout metadata plus directory/file stat snapshots in a process-wide cache shared across `Scanner` instances, so unchanged repeated scans can skip the recursive `ReadDir` walk and rebuild a fresh `Layout` directly from cached metadata while reusing validated checksum and Git metadata results; repo-state changes invalidate that snapshot conservatively | Confirmed by code, tests, and repeated-apply runtime |
| Shared inspector cache across fresh `Inspector` instances | `internal/db/inspector_impl.go` now caches `Inspect` results process-wide per connection/scope generation, and `internal/apply/apply.go` invalidates that cache after successful DB changes so repeated plan/apply loops do not re-query schema/object/column state once the database is stable | Confirmed by code, tests, and repeated-apply profiling |
| Batched audit history writes on apply success path | `internal/apply/apply.go` now publishes one `[]*types.ObjectEvent` payload per successful transactional batch, and `internal/audit/subscriber.go` inserts those records with one multi-row `INSERT ... VALUES` statement instead of one `ExecContext` per object | Confirmed by code, tests, and focused `pprof` on `TestLoad_StressApply` |
| Cached checksum history lookups | `internal/audit/load.go` now prefers static `OPENJSON` SQL for `LoadChecksums` and caches result maps in-process until `internal/audit/subscriber.go` writes new history rows for that connection | Confirmed by code, tests, and repeated-apply `pprof` |
| Static `OPENJSON` inspector reads with chunked fallback | `internal/db/inspector_impl.go` now probes `sys.databases.compatibility_level` once, marshals schema or object name sets to JSON once per inspector call, and runs static `internal/db/sql/*_openjson.sql` when compatibility level is `>= 130`; the older `types.BuildINQuery` / `BuildDualINQuery` path remains for fallback and non-inspector callers. `BuildDualINQuery` still emits repeated placeholders in one pass instead of chained `strings.Replace`, returns `[]string` args, and inspector fallback queries reuse precompiled plans for `objectSQL` / `columnSQL` before calling `driver.Conn.QueryStringsContext` or `QueryStringSlicesContext` | Confirmed by code, unit tests, and focused `benchmem`; see `docs/perf/runs/2026-05-18-openjson-inspector/README.md` |
...
| Inspector cache key materialization | `scopeKey()` in `internal/db/inspector_impl.go` collects **`[]scopePart` `{kind byte, s string}`**, sorts with **`sort.Slice`** (tuple order matches legacy `"kind:"+s` lexicographic order), then emits once via **`strings.Builder`**. **`Inspect`** keys the per-inspector `cache` map with **`scopeKeySHA256Hex`** (`hex(SHA256(UTF-8(canonical)))`, or `""` when canonical is empty) and passes that digest into **`sharedScopeCacheKey`** so long-lived map keys stay short. | Confirmed by code, goldens, `TestScopeKeySHA256HexGoldenVectors`, and `BenchmarkScopeKey*` |
...
| `inspector.cache` may grow without bound or retain temporary errors | `internal/db/inspector_impl.go` stores one `cachedScope` per **digest slot key** (`scopeKeySHA256Hex(canonical)` or `""`), and `sync.Once` caches both success and failure | The semantic risk is clear, but production impact depends on scope churn and engine lifetime |
...
| 3 | `BenchmarkScopeKey_2000Parts`, `BenchmarkScopeKeyPhase3SlotKey_2000Parts`, and `BenchmarkBuildDualINQuery_500x500` | `internal/db/inspector_bench_test.go`, `internal/types/chunk_bench_test.go` | Implemented. `scopeKey` uses sorted `scopePart` rows + `strings.Builder` (see `docs/perf/scopekey-optimization-plan.md`); **`Inspect`** uses **`scopeKeySHA256Hex`** for cache map keys. `BuildDualINQuery` now expands both placeholder sets in one pass over the SQL template and returns `[]string` args; inspector object/column queries reuse precompiled plans, and `driver.Conn.QueryStringsContext` / `QueryStringSlicesContext` delay boxing until the concrete driver boundary while avoiding an intermediate combined args slice on the dual-IN hot path. |
| 4 | `BenchmarkScannerPreloadGitInfo_200Paths` and `BenchmarkScannerPreloadGitInfo_200Paths_5kExtraGitFiles` | `internal/fs/scanner_bench_test.go` | Implemented; requires `git` on `PATH`. Fast path: newline walk on raw `git log` bytes, `parseBatchedGitLogCommitLine`, direct `Path` → `*CachedFile` targets, in-place `\`→`/`, first-match preload, repo-state keyed cross-scan cache reuse, and early exit once all layout paths are filled. Non-git roots skip the old per-file fallback because no metadata can be recovered there — see `docs/perf/scanner-preload-gitinfo-plan.md`. **`pprof`:** `docs/perf/runs/2026-05-18-scanner-preload-gitinfo-profile/README.md` |
...
- Failure mode: the first `Inspect()` for a scope fails because the database is temporarily unavailable.
  Containment: `internal/db/inspector_impl.go` caches the first error through `sync.Once`; recovery requires a new `inspector` instance or a different **cache slot key** (different layout digest, or `InvalidateInspectorCache` generation bump).
...
- Spot checks executed for this audit on `darwin/arm64` with `cpu: Apple M2`:
  ...
  - **Measured backlog benchmarks (2026-05-18)** — `darwin/arm64`, `cpu: Apple M2`, `go test` with `-benchmem -count=5 -benchtime=150ms` on `internal/bus`, `internal/db`, `internal/fs`, and `internal/apply`. Full stdout: `docs/perf/runs/2026-05-18-audit-bench/bench-output.txt`. Same run is reproduced by `make bench-perf-audit`.
    ...
    - `BenchmarkBuildDualINQuery_500x500`: on the real `internal/db/sql/objects.sql` template, ~`58.5–59.0 µs/op`, ~`27.3 KiB/op`, ~`1 alloc/op`; on the simpler `internal/types/chunk_bench_test.go` template, ~`21.5–26.4 µs/op`, ~`24.7 KiB/op`, ~`4 allocs/op`. On the `db` hot path, the remaining alloc is basically the final SQL string; string→`any` boxing starts later at the concrete driver boundary (`internal/driver/mssql`, then `database/sql`).
    - `BenchmarkInspectorInspect_Cold_500Objects`: ~`95.3–98.8 µs/op`, ~`193 KiB/op`, `50 allocs/op` on the `OPENJSON`-capable mock harness (`internal/db/inspector_bench_test.go` now returns `openjson_supported = 1` instead of benchmarking an accidental probe miss + fallback path)
    - `BenchmarkInspectorInspect_HotCache_500Objects`: ~`22.6–23.8 µs/op`, ~`25.9 KiB/op`, ~`5 allocs/op` (no additional `MockConn` queries after the initial cold `Inspect`)
    - `BenchmarkScannerPreloadGitInfo_200Paths`: prefer the steady capture (`-benchtime=3s`) after the direct-target pass: ~`16.4–16.5 ms/op`, ~`76 KiB/op`, ~`119 allocs/op` (`darwin/arm64`; requires `git`). Short-window `b.N==1` runs are still dominated by subprocess/setup noise.
    - `BenchmarkScannerPreloadGitInfo_200Paths_5kExtraGitFiles`: wall time remains noisy because `git` dominates at `b.N=1`, but allocator shape improved to roughly `3.2 MiB/op` and `31.6k allocs/op` on the same synthetic 5k-noise repo; see `docs/perf/scanner-preload-gitinfo-plan.md`.
