...
- Planning and managed-scope validation also limit successful metadata checksum reads to the normalized keys present in the resolved layout or affected validation subset, so normal runs do not scan the full `[__migrator].[object_state]` table.
...
- Metadata run start uses `internal/migrator/metadata_context.go` and respects the active command cancel/deadline. Post-SQL metadata writes and run finalization use the same file's short cleanup timeout so failure recording and finish-state updates can still complete quickly after the main command context is canceled.
- When `--report-dir` is set, `internal/reports/write.go` stages report artifacts through temporary files, publishes JSON first, and publishes the text companion last as the commit marker for readers that require a consistent pair.
...
- When `--plan-file` is supplied, `migrate` reads and validates the approved plan artifact once, rebuilds the current plan from the live catalog and metadata to verify the approval boundary, includes transition-backed `objects[].transition_paths` in that approved object-set comparison, and skips transition/scaffold generation preflight on that approval path so the repo tree is not mutated during approved execution.
...
- For tracked table drift, that text view includes the checked-in transition paths that `migrate` will apply instead of replaying the repo table SQL against the existing live table, or the required transition directory when the change is still blocked.
...
- Approved-plan drift: when `--plan-file` is supplied, `migrate` fails closed if `git_commit`, `layout_hash`, target, tool identity, comparison mode, update policy, transaction mode, rollback scope, base selection, or the approved schema/object set differs, including checked-in transition paths on transition-backed tracked tables.
...
- Execution failure after reportable work: when `--report-dir` is set, `migration-report.*` is still finalized through the normal classified failure envelope before the command returns the execution error.
