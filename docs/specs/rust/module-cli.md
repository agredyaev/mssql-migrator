...
- `crates/cli/src/main.rs` - argument parsing, config load, command dispatch
...
- Inputs: argv, typed TOML, `RM_*` / `RMIG_*` process environment
...
- Constraint: `version` does not load `config.toml` or connect to SQL.

## Nominal flow

1. Parse command and flags (`--config`, `--json`).
...
- Log filter defaults: `crates/core/src/tests/log_filter_test.rs`
