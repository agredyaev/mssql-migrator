...
const TABLE_SQL: &str = "dactests/smoke/tables/smoke_table.sql";
const VIEW_SQL: &str = "dactests/smoke/views/smoke_view.sql";
...
async fn workflow_git_scenarios_single_session() {
    ...
    let mut conn = state_smoke::open_conn(cfg).await.expect("connect");
    state_smoke::assert_smoke_objects_materialized(&mut conn)
        .await
        .expect("smoke objects");
    let audit = state_smoke::count_audit_rows(&mut conn, "object", "applied")
        .await
        .expect("audit count");
    ...
    git.add_tree_and_commit(
        "dactests/smoke/tables/_migrations/smoke_table",
        "test: track transition migration",
    )
    ...
    let audit_after_ddl = state_smoke::count_audit_rows(&mut conn, "object", "applied")
        .await
    ...
    let audit_after_view = state_smoke::count_audit_rows(&mut conn, "object", "applied")
        .await
