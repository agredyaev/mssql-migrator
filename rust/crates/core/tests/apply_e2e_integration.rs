...
async fn apply_e2e_cold_migrate_creates_objects_and_audit() {
    ...
    let mut conn = state::open_conn(&cold).await.expect("connect");
    state::assert_smoke_objects_materialized(&mut conn)
        .await
        .expect("smoke objects in sys catalog");

    let audit = state::count_audit_rows(&mut conn, "object", "applied")
        .await
        .expect("audit count");
    assert!(
