...
/// Catalog-relative SQL path: `{database}/{schema}/...` (matches Go `fs.Object.Path` / `TransitionScript.Path`).
pub fn with_database_prefix(database: &str, path: &str) -> SharedStr {
    object_path_for_entry(database, share(path.replace('\\', "/")))
}

/// Build once at scan finalize (**CASE-2**); hot fill clones the handle only.
