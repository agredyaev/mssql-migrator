...
1. **Daemon head-of-line blocking** — one warm TDS connection serializes all clients (by design; bounded by 60 s idle timeout, command timeout, `MAX_DAEMON_CLIENTS`). Throughput ceiling, not a correctness risk.
