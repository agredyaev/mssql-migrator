...
| 100 000 | 13 458 s (3 h 44 m) | 7.4/s | 443 MB | 321.7 s (5.4 min) | 539 MB |

**Correctness held at 100k**: after applying all 100 000 tables the audit history had exactly 100 000 rows and the re-plan reported exactly 100 000 `skip_unchanged` with both commands exiting 0 — no corruption, no partial state, no crash. The tool is functionally correct at 100k; the open questions at that scale are time and memory, not correctness.

Readback peak RSS is linear (~5.3 KB/object: 21 → 120 → 539 MB) — bounded and container-friendly, from full materialization of result sets (`Vec<Vec<RowData>>`). Readback wall at 20k decomposes as **checksum+drift query 27.5 s (73 %)**, catalog query 9.3 s (25 %), file scan 0.6 s, `compute_diff` 0.05 s, connect 6 ms — **98 % is SQL Server round-trip, and the single dominant cost is the per-object live-definition drift fingerprint** (`sql/audit/_object_canonical_state.sql`), computed for every workspace object on every plan (`keys_json` is the full workspace key set; git-delta scoping applies only to catalog inspection). Plan time is therefore O(catalog size): sub-second for the product's real target (hundreds to low-thousands of objects), tens of seconds at 20k, 5.4 minutes at 100k.

Two scale caveats fall out of the 100k run, both under emulation:

- **Apply throughput degrades as the catalog grows** — 26.5 → 41.8 → **7.4 obj/s** (the 100k apply took 3 h 44 m). Each `CREATE TABLE` mutates system catalogs that are themselves growing, and the whole run is CPU-bound at ~99 % under Rosetta (amd64 SQL Server on arm64). Separating genuine O(N) catalog-metadata degradation from the emulation tax requires a native-amd64 measurement, which this host cannot provide; on native hardware apply would be materially faster. This is an operational note, not a correctness defect.
- **The default 30 s `RM_COMMAND_TIMEOUT` is too tight at this scale** — a full 100k plan with the default fails deterministically with `rmig: query timed out after 30s` (exit 5, at 34 s), because the drift query alone needs minutes. The same plan with `RM_COMMAND_TIMEOUT=600` succeeds (321.7 s). The error is actionable and the knob is documented (`docs/ci-usage.md`, `docs/perf-footprint-audit.md`).
...
2. ~~**No metrics/health endpoint** on rmigd~~ **Closed.** The daemon answers `{"op":"stats"}` on its socket with `{uptime_s, requests, reconnects, queue_waits, warm_connection}` (`session/daemon/metrics.rs`), touching no SQL Server, so it serves as both a metrics pull and a liveness/readiness probe; reconnects and warm-session queue waits are also logged via `tracing`. Covered by `rmigd_stats_test` (endpoint) + a metrics-format unit test. A Prometheus/HTTP exporter is still out of scope (this is a local-socket CI helper, not a scraped service).
3. ~~**Arena invariant `panic!`s under `panic=abort`**~~ **Closed
   2026-07-24.** The arena and offset representation were removed; workspace
   metadata now uses owned values and exact keys (ADR-0003).
...
10. **Local incremental builds can carry a stale `RMIG_COMMIT` stamp** —
    `build.rs` stamps the commit when `migrator-core` compiles, so a CLI-only
    incremental rebuild can retain the previous core stamp. CI release builds
    compile the validated checkout. The package version comes only from
