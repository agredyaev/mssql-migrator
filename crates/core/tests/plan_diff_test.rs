...
use migrator_core::plan::{compute_diff, compute_diff_into};
...
fn changed_index_without_safe_update_path_is_blocked_before_apply() {
    ...
    let script_id = ws.insert_script(Script {
        ...
        kind: ScriptKind::Object,
        abs_path: "r/indexes/ix_t1.sql".into(),
        checksum: Some([1; 32]),
    });
    let db_id = ws.intern_database("db".into());
    ws.adopt_dense_entries(vec![ObjectEntry::new(
...
#[test]
fn unchanged_diff_reuses_planned_object_storage() {
    let mut ws = Workspace::default();
    let key = ObjectKey::new("r", "tables", "t1");
    let script_id = ws.insert_script(Script {
        key: ScriptKey::from_path("r/tables/t1.sql"),
    ...
    let mut checksums = ChecksumMap::new();
    checksums.insert_key(&key, [1; 32]);
    let mut catalog = CatalogState::default();
    catalog
        .objects
        .insert(key, catalog_object("r", "tables", "t1", None));
    let mut plan = MigrationPlan::default();

    compute_diff_into(&mut ws, &catalog, &checksums, &mut plan).unwrap();
    let objects = plan.objects.as_ptr();
    let key = plan.objects[0].normalized_key.as_ptr();
    let path = plan.objects[0].object_path.as_ptr();
    compute_diff_into(&mut ws, &catalog, &checksums, &mut plan).unwrap();

    assert_eq!(plan.objects.as_ptr(), objects);
    assert_eq!(plan.objects[0].normalized_key.as_ptr(), key);
    assert_eq!(plan.objects[0].object_path.as_ptr(), path);
}
