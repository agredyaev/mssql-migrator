...
fn cleanup_smoke_scaffold() {
    ...
    let catalog_db = catalog::sole_catalog_database(&cfg.sql_root).unwrap_or_else(|e| {
        panic!("catalog database under {}: {e}", cfg.sql_root);
    });
    let dir = sql_root
        .join(&catalog_db)
    ...
    let _ = std::fs::remove_dir(sql_root.join(&catalog_db).join("smoke/tables/_migrations"));
}
