...
pub fn refusal_for(req: &Request, ep: &Endpoint) -> Option<Response> {
    ...
    {
        if let Some(msg) =
            endpoint_mismatch(ep, server, port, user, *encrypt, *trust_server_certificate)
        {
...
pub fn endpoint_mismatch(
    ...
    let mut mismatched: Vec<&str> = pairs
        .iter()
        .filter(|(_, got, want)| !got.is_empty() && got != want)
        .map(|(name, _, _)| *name)
        .collect();
    if encrypt.is_some_and(|value| value != ep.encrypt) {
        mismatched.push("encrypt");
    }
    if trust_server_certificate.is_some_and(|value| value != ep.trust_server_certificate) {
        mismatched.push("trust_server_certificate");
    }
