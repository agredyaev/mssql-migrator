...
/// Removes already-applied transition paths from `plan`. An applied path whose current file checksum no
...
pub fn filter_applied_migrations_on_plan(
    ...
    filter_objects(plan, applied, &current, &mut tampered);
...
fn filter_objects(
    ...
    for obj in &mut plan.objects {
        ...
        obj.transition_paths
            .retain(|path| !applied_unchanged(path, applied, current, tampered));
    }
}
