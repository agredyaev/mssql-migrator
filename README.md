...
- `migrate` executes create paths and safe existing-module update paths from the current in-memory plan, treats `adopt_existing` as a no-DDL adoption path, records attempts into `[__migrator]`, and limits post-migrate validation to affected managed-object existence checks and metadata success-state updates without module refresh work.
- `plan`, `migrate`, and managed-scope `validate` load successful metadata checksums only for objects in the resolved repo layout or affected validation subset; they do not scan the full `[__migrator].[object_state]` set during normal scoped runs.
...
- When `--report-dir` is set, report files are written through `internal/reports/write.go` by staging `*.tmp` files, publishing JSON first, and publishing the text companion last as the commit marker for readers that require a consistent pair.
- Metadata run start uses the active command context in `internal/migrator/metadata_context.go`. Post-SQL metadata updates and run finalization use a short bounded cleanup context so metadata/report cleanup can finish without waiting for the full command timeout.
